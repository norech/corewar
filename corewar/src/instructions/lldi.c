...
bool lldi(runtime_op_t *op UNUSED, vm_t *vm UNUSED,
    program_memory_t *instance)
{
    instance->registers[ op->args[2].reg_id - 1 ]
        = resolve_arg_index_value(&op->args[0], instance, false, false)
            + resolve_arg_index_value(&op->args[1], instance, false, false);
